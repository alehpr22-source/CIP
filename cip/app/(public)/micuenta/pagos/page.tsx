        ...
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border bg-white p-6 text-center">
              ...
              <p className={`mt-1 text-lg font-bold ${deudaResult.data.estadoHabilitacion === "Habilitado" ? "text-green-600" : "text-red-600"}`}>
                {deudaResult.data.estadoHabilitacion === "Habilitado" ? "✅ Habilitado" : "❌ Inhabilitado"}
              </p>
            </div>
            <div className="rounded-lg border bg-white p-6 text-center">
          ...
          {deudaResult.data.mesesAdeudados.length > 0 && (
            <div className="rounded-lg border bg-white p-6">
              <h2 className="mb-4 text-lg font-semibold text-gray-800">📋 Meses pendientes</h2>
              ...
              <div className="mt-4 border-t border-gray-200 pt-4">
                <p className="mb-2 text-sm text-gray-600">
                  Puedes pagar el mes actual <strong>{nombreMes(deudaResult.data.mesActual)} {deudaResult.data.anioActual}</strong> por S/ 20.00.
                  Para regularizar tu situación, puedes ponerte al día con todos los meses pendientes.
                </p>
                <PagarForm
                  colegiadoId={deudaResult.data.colegiadoId}
                  anio={deudaResult.data.anioActual}
                  mes={deudaResult.data.mesActual}
                />
              </div>
            </div>
          )}
